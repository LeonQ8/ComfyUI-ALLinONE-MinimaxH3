...
**Live Preview** (the toggle under the video): needs `taeh3.safetensors` in `ComfyUI/models/vae_approx/` ([Kijai/MiniMax-H3-TAE](https://huggingface.co/Kijai/MiniMax-H3-TAE)) and nothing else. The clip plays in the preview box while it samples, decoded with the tiny TAEH3 model; a quality dropdown next to the toggle picks Fast, Balanced or Detailed previews, so you can trade preview smoothness against generation speed. Works in every video mode, not with the Turbo preset or Image mode. Your copy can sit in a subfolder of `vae_approx`, pick it under Settings: Live Preview decoder. Tested on ComfyUI 0.32.
